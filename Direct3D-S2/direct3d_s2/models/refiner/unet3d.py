    ...
    def forward(
        ...
        for downsample_block in self.down_blocks:
            if self.use_checkpoint:
                # Wrapper to pass hidden_states as keyword argument
                def down_checkpoint_wrapper(block, h_states):
                    return block(hidden_states=h_states)
                sample, res_samples = torch.utils.checkpoint.checkpoint(
                    down_checkpoint_wrapper, downsample_block, sample, use_reentrant=False
                )
            else:
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if self.use_checkpoint:
                # Wrapper function to properly pass arguments to checkpoint
                def checkpoint_wrapper(block, h_states, res_states, up_size):
                    return block(
                        hidden_states=h_states,
                        res_hidden_states_tuple=res_states,
                        upsample_size=up_size
                    )
                sample = torch.utils.checkpoint.checkpoint(
                    checkpoint_wrapper, upsample_block, sample, res_samples, upsample_size, use_reentrant=False
                )
            else:
