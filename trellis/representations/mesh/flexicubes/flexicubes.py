...
# Replacement for kaolin.utils.testing.check_tensor
# This avoids the dependency on kaolin which is difficult to install on Windows
def check_tensor(tensor, expected_shape, throw=True):
    """
    Simple tensor shape validation to replace kaolin.utils.testing.check_tensor
    Args:
        tensor: PyTorch tensor to check
        expected_shape: tuple where None means any size, e.g., (None, 3) means (?, 3)
        throw: if True, raises exception on mismatch; if False, returns bool
    Returns:
        bool: True if shape matches, False otherwise (when throw=False)
    """
    if not torch.is_tensor(tensor):
        if throw:
            raise ValueError(f"Expected a tensor, got {type(tensor)}")
        return False
    ...
    if len(actual_shape) != len(expected_shape):
        if throw:
            raise ValueError(f"Expected {len(expected_shape)} dimensions, got {len(actual_shape)}")
        return False
