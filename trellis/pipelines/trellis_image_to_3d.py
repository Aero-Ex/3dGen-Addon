...
class TrellisImageTo3DPipeline(Pipeline):
    ...
    def encode_image(self, image: Union[torch.Tensor, list[Image.Image]]) -> torch.Tensor:
        ...
        # Use offloading context manager for image conditioning model
        with self._temporary_gpu_model('image_cond_model') as model:
            # Ensure image is on same device as model (GPU when offloading, otherwise self.device)
