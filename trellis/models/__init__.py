    ...
    if name not in globals():
        if name in __attributes:
            ...
            try:
                ...
                module = importlib.import_module(f"trellis.models.{module_name}")
            globals()[name] = getattr(module, name)
        elif name in __submodules:
            try:
                module = importlib.import_module(f".{name}", __name__)
            except (ImportError, ValueError):
                module = importlib.import_module(f"trellis.models.{name}")
            globals()[name] = module
        else:
...
def from_pretrained(path: str, **kwargs):
    ...
    if is_local:
        ...
        # Token can be provided via HF_TOKEN or HUGGING_FACE_HUB_TOKEN environment variable
        token = os.environ.get('HF_TOKEN') or os.environ.get('HUGGING_FACE_HUB_TOKEN')

        config_file = hf_hub_download(repo_id, f"{model_name}.json", token=token)
        model_file = hf_hub_download(repo_id, f"{model_name}.safetensors", token=token)

    with open(config_file, 'r') as f:
        config = json.load(f)
    model = __getattr__(config['name'])(**config['args'], **kwargs)
    
    # Load state dict directly - spconv uses standard PyTorch naming (.weight)
