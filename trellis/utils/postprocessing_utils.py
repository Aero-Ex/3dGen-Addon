...
def postprocess_mesh(
    ...
    if simplify and simplify_ratio > 0:
        ...
        # FIX: pyvista has initialization bug - attributes accessed but never initialized
        # This causes AttributeError in copy_meta_from() during decimate()
        # The info tuples need to be mutable (lists) so copy() works
        if not hasattr(mesh, '_association_complex_names'):
            mesh._association_complex_names = {}
        if not hasattr(mesh, '_association_bitarray_names'):
            mesh._association_bitarray_names = {}
        if not hasattr(mesh, '_active_scalars_info'):
            mesh._active_scalars_info = [None, None]
        if not hasattr(mesh, '_active_vectors_info'):
            mesh._active_vectors_info = [None, None]
        if not hasattr(mesh, '_active_tensors_info'):
            mesh._active_tensors_info = [None, None]
        
        # Fix for PointData/CellData missing active attribute names
        for attr in [mesh.point_data, mesh.cell_data]:
            if not hasattr(attr, 'active_scalars_name'):
                attr.active_scalars_name = None
            if not hasattr(attr, 'active_vectors_name'):
                attr.active_vectors_name = None
            if not hasattr(attr, 'active_tensors_name'):
                attr.active_tensors_name = None
        
        mesh = mesh.decimate(simplify_ratio, progress_bar=verbose)
        from vtkmodules.util.numpy_support import vtk_to_numpy
        vertices = vtk_to_numpy(mesh.points.GetData())
...
def bake_texture(
    ...
    if mode == 'fast':
        ...
        total_steps = 2500
        # Use torch random instead of numpy to avoid CPU-GPU sync
        import torch as _torch
        with tqdm(total=total_steps, disable=not verbose, desc='Texture baking (opt): optimizing') as pbar:
            for step in range(total_steps):
                optimizer.zero_grad()
                selected = _torch.randint(0, len(views), (1,), device='cpu').item()  # Faster than np.random
